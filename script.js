...
function ensureAuthForDashboard() {
  ...
  const dashboardButton = document.querySelector('[data-go-dashboard]');
  if (dashboardButton) {
    if (currentEmail) {
  ...
  if (page === 'dashboard' && !currentEmail) {
  ...
  if (signUpForm) {
    signUpForm.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        storage.setCurrentUserEmail(email);
        showToast('Passkey registered! Redirecting to dashboard...', 'success');
